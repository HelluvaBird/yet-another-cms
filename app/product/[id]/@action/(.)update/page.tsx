...
export default function UpdatePage({ params: { id } }: Props) {
  ...
  const productData = queryClient.getQueryData<ProductsRecord>(['product', id]);
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (media) {
      ...
      if (productData?.media) {
        mediaUrls.push(...productData?.media);
      }
    }

    mutation.mutate({ thumbnailUrl, mediaUrls });
  };
  return (
    <div
    ...
    >
      <div className="p-4 bg-white w-full max-w-xl rounded space-y-8">
        <div className="flex justify-between items-center border-b pb-4">
          <p className="text-xl">Update Product</p>
          <button
            type="button"
            onClick={() => router.back()}
